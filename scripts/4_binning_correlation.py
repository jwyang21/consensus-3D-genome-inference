...
if __name__ == '__main__':
    ...
    for i in range(bin_num):
        ...
        bin_name = 'chr'+args.chrom+':'+str(args.binsize*i)+'-'+str(args.binsize*(i+1)) #[start, end)
    ...
    ## 2-2. save binned CpG probe as pickle file
    bin_probe_pickle = os.path.join(args.save_dir, f'bin_probe_GRCh{args.grch}_chr{args.chrom}_{args.cpg_type}.pickle')
    with open(bin_probe_pickle, 'wb') as f:
        pickle.dump(bin_probe, f) 
    f.close()
    ...
    with open(bin_with_probes_list_pickle, 'wb') as f2:
        pickle.dump(bin_with_probes, f2)
    f2.close()

    bin_with_probes_index_pickle = os.path.join(args.save_dir, f'bin_with_probes_index_list_GRCh{args.grch}_chr{args.chrom}_{args.cpg_type}.pickle')
    with open(bin_with_probes_index_pickle, 'wb') as f3:
        pickle.dump(bin_with_probes_index, f3)
    f3.close()
